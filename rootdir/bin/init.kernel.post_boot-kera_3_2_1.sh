...
if [ -d /proc/sys/walt ]; then
	...
	echo 8500000 8500000 8500000 5000000 5000000 2000000 > /proc/sys/walt/sched_util_busy_hyst_cpu_ns
	echo 255 > /proc/sys/walt/sched_util_busy_hysteresis_enable_cpus
	echo 1 1 1 15 15 15 > /proc/sys/walt/sched_util_busy_hyst_cpu_util
	echo 40 > /proc/sys/walt/sched_cluster_util_thres_pct
	echo 30 > /proc/sys/walt/sched_idle_enough

	# Enable Gold CPUs for pipeline
	echo 96 > /proc/sys/walt/sched_pipeline_cpus
	...
	echo 1113600 0 0 0 0 0 > /proc/sys/walt/input_boost/input_boost_freq
	...
	# configure walt boost for gold and gold+
	echo -6 > /sys/devices/system/cpu/cpufreq/policy3/walt/boost
	echo -6 > /sys/devices/system/cpu/cpufreq/policy5/walt/boost
...
echo "0:595200 3:633600 5:633600" > /data/vendor/perfd/default_scaling_min_freq
